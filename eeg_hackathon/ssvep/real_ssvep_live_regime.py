...
def run(subjects=tuple(range(1, 10)), win_s=2.0, n_targets=4, require_n=9):
    ...
    nsamp = int(win_s * fs)
    print(f"Nakanishi2015 — LIVE-REGIME FBCCA: {win_s}s window, {n_targets}-class subset "
          f"(matches the 2048 game), n up to {len(subjects)}:\n")
    accs, missing = [], []
    for s in subjects:
        try:
        ...
        Xs, ys = X[mask][:, :, :nsamp], yi[mask]          # 2 s window + 4-class subset
        lut = {p: k for k, p in enumerate(pick)}
        correct = np.mean([classify(Xs[k], freqs=sub_freqs, fs=fs)[0] == lut[ys[k]]
                           for k in range(len(Xs))])
        accs.append(correct)
        print(f"  S{s}: {len(Xs)} trials, {n_targets}-class, {win_s}s -> FBCCA acc={correct:.2f}")
    accs = np.array(accs)
    print(f"\n  LIVE-REGIME FBCCA: mean={accs.mean():.2f} +/- {accs.std():.2f}  "
          f"(n={len(accs)}, {n_targets}-class, {win_s}s, 0-train, chance={1/n_targets:.2f})")
